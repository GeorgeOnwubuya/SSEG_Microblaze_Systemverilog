...
(
   ...
   generate
      genvar i;
      for (i=4; i<7; i=i+1) begin:  unused_slot_gen_4to7
         assign rd_data_array[i] = 32'hffffffff;
      end
   endgenerate
    
   //Slot 8: Seven Segment
   my_sseg #(.W(N_SSEG)) sseg_slot8
   (.clk(clk),
    .reset(reset),
    .cs(cs_array[`S8_SSEG]),
   ...
   generate
      genvar i_nxt;
      for (i_nxt=9; i_nxt<64; i_nxt=i_nxt+1) begin:  unused_slot_gen_9to63
         assign rd_data_array[i] = 32'hffffffff;
      end
   endgenerate
endmodule
